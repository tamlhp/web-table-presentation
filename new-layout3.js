            ...
            if(cls != 'new-tops' && cls.indexOf("new-top") >= 0){
                ...
                for(i = 0; i < rows.length; i++){
                    if(top >= rows.length){
                        ...
                        var data = rows[i].getAttribute("data-value");
                        console.log(rows[i], data);
                        rows[i].style.display = data.indexOf("[" + top + "]") > 0?"":"none";
                    }
    ...
    var buildTable = function(table, keyword){
        ...
        for(var i = 0; i < tops.length; i++){
            if(tops[i] + header >= rows.length){
                ...
                topNames += '<div class="new-top {0}" data-value="{1}">Show {2}</div>'.format(i == 0?"new-top-active":"", tops[i] + header, tops[i]);
                tops[i] += header;
            }
        }
...
//            '<div class="google-show-more">Show <span class="more">more</span> ({3} rows / {4} columns total) - Import data</div>' +
        ...
        for(var i = 0; i < rows.length; i++){
            row = rows[i];
            var top = row[row.length - 1];

            result += "<tr data-value='{0}' style='{1}'>".format(top, top.indexOf("##[1]") < 0 ? "display:none":"");
