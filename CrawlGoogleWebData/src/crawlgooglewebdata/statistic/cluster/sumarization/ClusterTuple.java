...
//                list100.add(s);
        ...
        for (SSchema s : list100) {
            summarization(s, true, new SingleLinkageStrategy());
//            break;
        }
    }

    public static void summarization(SSchema schema, boolean random, LinkageStrategy linkage) {
...
//        System.out.println("get k clusters");
        ...
        for (runtimeCount = 0; runtimeCount < (random ? 100 : 1); runtimeCount++) {
            ...
            for (int i = 5; i <= 50; i += 5) {
                ...
                times[(i - 5) / 5] = System.nanoTime() - t0;
                if (!random) {
                ...
                for (Cluster c : leafs) {
                    if (!setClusterLeafs.contains(c)) {
                        ils[(i - 5) / 5] += singleLinkage(clusteringAlgorithm, c, kclusters);
                    }
                }
                double pt = 0;
